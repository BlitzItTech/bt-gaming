// Complete mapping of all 66 books of the Bible, their chapter counts, 
// and the precise number of verses per individual chapter.
const completeBibleData = [
  // --- OLD TESTAMENT ---
  { name: "Genesis", chapters: [31,25,24,26,32,22,24,22,29,32,32,20,18,24,21,16,27,33,38,18,34,24,20,67,34,35,46,22,35,43,55,32,20,31,29,43,36,30,23,23,57,38,34,34,34,34,31,22,33,26] },
  ...
  { name: "Leviticus", chapters: [17,16,17,35,19,30,38,36,24,20,47,8,59,57,33,34,16,30,37,27,24,33,43,23,55,46,34] },
  { name: "Numbers", chapters: [54,34,51,49,31,27,89,26,23,36,35,16,33,45,41,50,13,32,22,29,35,41,30,25,19,65,23,31,40,16,54,42,56,29,34,13] },
  ...
  { name: "2 Chronicles", chapters: [17,18,17,22,14,16,22,18,31,19,23,16,22,15,19,14,19,34,11,37,20,12,21,27,28,23,9,27,36,27,21,33,25,33,27,23] },
  ...
  { name: "Job", chapters: [22,13,26,21,27,30,21,22,35,22,20,25,28,22,35,22,16,21,29,29,34,30,17,25,6,14,23,28,25,31,40,22,33,37,16,33,24,41,30,32,34,17] },
  { name: "Psalms", chapters: [6,12,8,8,12,10,17,9,20,18,7,8,6,7,5,11,15,50,14,9,13,31,6,10,22,12,14,9,11,12,24,11,22,22,28,12,40,22,13,17,13,11,5,26,17,11,9,14,20,23,19,9,6,7,23,13,11,11,17,12,8,12,11,10,13,20,7,35,36,5,24,20,23,10,12,20,72,13,19,16,8,18,12,13,17,7,18,52,17,16,15,5,23,11,13,12,9,9,5,8,28,22,35,45,48,43,13,31,7,10,10,9,8,18,19,2,29,176,7,8,9,4,8,5,6,5,6,8,8,3,18,3,3,21,26,9,8,24,13,10,7,12,15,21,10,20,14,9,6] },
  ...
  { name: "Isaiah", chapters: [31,22,26,6,30,13,25,22,21,34,16,6,22,32,9,14,14,7,25,6,17,25,18,23,12,21,13,29,24,33,9,20,24,17,10,22,38,22,8,31,29,25,28,28,25,13,15,22,26,11,23,12,12,17,13,12,21,14,21,22,11,12,19,12,25,24] },
  ...
  { name: "Luke", chapters: [80,52,38,44,39,49,50,56,62,42,54,59,35,35,32,31,37,35,48,47,38,71,56,53] },
  { name: "John", chapters: [51,25,36,54,47,71,53,59,41,42,57,50,38,31,27,33,26,40,42,31,25] },
  { name: "Acts", chapters: [26,47,26,37,42,15,60,40,43,48,30,25,52,28,41,40,34,28,41,38,38,30,35,27,27,32,44,31] },
...
// Function to generate a completely random, valid Bible reference
export function getRandomBibleReference(bibleDataset = completeBibleData) {
  // 1. Pick a random book object from the 66 books
  const randomBook = bibleDataset[Math.floor(Math.random() * bibleDataset.length)];
  
  // 2. Pick a random chapter index based on how many chapters that book has
  const randomChapterIndex = Math.floor(Math.random() * randomBook.chapters.length);
  const randomChapter = randomChapterIndex + 1;
  
  // 3. Look up the exact max verse count for that specific chapter
  const maxVerseCount = randomBook.chapters[randomChapterIndex];
  
  // 4. Pick a random verse safely within that chapter's limit
  const randomVerse = getRandomInt(1, maxVerseCount);
...
// Example usage:
console.log(getRandomBibleReference().reference); 
// Output format example: "Romans 8:28" or "Psalms 119:105"